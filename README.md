...
## GitHub Agentic Workflow Quickstart

This repository also includes a small lab for cost-controlled GitHub Agentic
Workflows. It shows how Copilot-powered workflows can use bounded turns,
AI-credit caps, and safe outputs without requiring contributors to read
...
6. Preview issue triage against an issue context. The `issue-triage` workflow is
   ...
   a safe preview. The illustrative command below requires authentication and a
   real issue number; check `gh aw trial --help` for your installed CLI version:
